...
## Homework 5: Introduction to hypothesis testing and *t*-tests (100 pts)
#### BIO5312
...
**2. (4 points)** Consider the following scenario: You perform a one-sample *t*-test with a sample size of $n=100$ and a significance level $\alpha=0.1$. Answer the following questions in this context:
...
**3. (6 points)** Consider the following scenario: You perform a one-sample *t*-test with a sample size of $n=100$ and a significance level $\alpha=0.1$. Answer the following questions in this context:
...
c) You perform a one-sided one-sample *t*-test and the data deviate in the wrong direction. What is the *minimum* P-value you could obtain in this scenario?
...
**4. (2 points)** Assume, for a given test, that the null hypothesis is actually _true_. Which one of the following statements is _true_?

+ *Statement 1*: A study with a larger sample size is *more* likely than a smaller study to get the result P < 0.05.
+ *Statement 2*: A study with a larger sample size is *less* likely than a smaller study to get the result P < 0.05.
+ *Statement 3*: A study with a larger sample size is *equally* likely compared to a smaller study to get
the result P < 0.05.
...
**5. (2 points)** Assume, for a given test, that the null hypothesis is actually _false_. Which one of the following statements is _true_?
...
**3. (10 pts)** Compute a 90% confidence interval for the test from question 1, using R as a calculator below. (Hint: to compute a 95% CI, we use $t_{0.025}$, with the appropriate degrees of freedom, to determine the limits. Think about which $t$ to use instead for a 90% CI).
...
a) What would the P-value have been for Question 2 if we had performed a **one-sided** *t*-test with the alternative hypothesis that Artiodactyla has, on average, *larger* litters than Cetacea?

`The P-value would have been: Answer goes here.`

b) What would the P-value have been for Question 2 if we had performed a **one-sided** *t*-test with the alternative hypothesis that Artiodactyla has, on average, *smaller* litters than Cetacea?

`The P-value would have been: Answer goes here.`

<br><br>

**6. (15 pts).** Perform a two-sided two-tailed paired *t*-test to address the question: Do cows (Order Artiodactyla) and whales (Order Cetacea) have different litter sizes on average? Carry out this test using the function `t.test()`.
...
**7 (10 pts).** Perform the same hypothesis test as in question 4 (only the call to `t.test()`), but this time run it as an independent two-sample *t*-test (still two-sided). Compare the test output between this approach and the paired approach taken in question 4, focusing on differences between the resulting P-value and confidence intervals. Explain why these values differ between approaches. What do these findings tell you about the relative *power* of using either approach to compare paired data? 
